...
import model.Buku;
import repository.RepositoryBuku;
...
public class ServicePerpustakaanTest {
    ...
    // Tambahkan method test berikut ke ServicePerpustakaanTest.java

    @Test
    @DisplayName("Test tambah buku dengan data null")
    void testTambahBukuNull() {
        boolean hasil = servicePerpustakaan.tambahBuku(null);
        assertFalse(hasil, "Harus gagal menambah buku null");
    }

    @Test
    @DisplayName("Test hapus buku dengan ISBN null")
    void testHapusBukuIsbnNull() {
        boolean hasil = servicePerpustakaan.hapusBuku(null);
        assertFalse(hasil, "Harus gagal menghapus dengan ISBN null");
    }

    @Test
    @DisplayName("Test cari buku dengan ISBN null")
    void testCariBukuByIsbnNull() {
        Optional<Buku> hasil = servicePerpustakaan.cariBukuByIsbn(null);
        assertFalse(hasil.isPresent(), "Harus empty untuk ISBN null");
    }

    @Test
    @DisplayName("Test pinjam buku dengan parameter null")
    void testPinjamBukuWithNullParameters() {
        assertFalse(servicePerpustakaan.pinjamBuku(null, anggotaTest));
        assertFalse(servicePerpustakaan.pinjamBuku("1234567890", null));
        assertFalse(servicePerpustakaan.pinjamBuku(null, null));
    }

    @Test
    @DisplayName("Test kembalikan buku dengan parameter null")
    void testKembalikanBukuWithNullParameters() {
        assertFalse(servicePerpustakaan.kembalikanBuku(null, anggotaTest));
        assertFalse(servicePerpustakaan.kembalikanBuku("1234567890", null));
        assertFalse(servicePerpustakaan.kembalikanBuku(null, null));
    }

    // Add this test method to ServicePerpustakaanTest.java

    @Test
    @DisplayName("Test cari buku by pengarang berhasil")
    void testCariBukuByPengarang() {
        // Arrange
        List<Buku> daftarBuku = Arrays.asList(bukuTest);
        when(mockRepositoryBuku.cariByPengarang("John Doe")).thenReturn(daftarBuku);

        // Act
        List<Buku> hasil = servicePerpustakaan.cariBukuByPengarang("John Doe");

        // Assert
        assertEquals(1, hasil.size());
        assertEquals("Pemrograman Java", hasil.get(0).getJudul());
        assertEquals("John Doe", hasil.get(0).getPengarang());
        verify(mockRepositoryBuku).cariByPengarang("John Doe");
    }

    @Test
    @DisplayName("Test cari buku by pengarang dengan string kosong")
    void testCariBukuByPengarangEmpty() {
        // Act
        List<Buku> hasil = servicePerpustakaan.cariBukuByPengarang("");
    ...
    @Test
    @DisplayName("Test buku tersedia dengan ISBN null")
    void testBukuTersediaWithNullIsbn() {
        assertFalse(servicePerpustakaan.bukuTersedia(null));
    }

    @Test
    @DisplayName("Test get jumlah tersedia dengan ISBN null")
    void testGetJumlahTersediaWithNullIsbn() {
        assertEquals(0, servicePerpustakaan.getJumlahTersedia(null));
    }
