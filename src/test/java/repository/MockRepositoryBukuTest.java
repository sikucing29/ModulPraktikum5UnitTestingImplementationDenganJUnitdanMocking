...
class MockRepositoryBukuTest {
    ...
    // Add these test methods to MockRepositoryBukuTest.java

    @Test
    @DisplayName("Test updateJumlahTersedia dengan jumlah sama dengan total")
    void testUpdateJumlahTersediaEqualToTotal() {
        mockRepository.simpan(buku1); // jumlahTotal = 5

        boolean hasil = mockRepository.updateJumlahTersedia("1234567890", 5);

        assertTrue(hasil);
        Optional<Buku> bukuUpdated = mockRepository.cariByIsbn("1234567890");
        assertTrue(bukuUpdated.isPresent());
        assertEquals(5, bukuUpdated.get().getJumlahTersedia());
    }

    @Test
    @DisplayName("Test cariByJudul dengan judul null pada buku")
    void testCariByJudulWithNullJudulInBook() {
        Buku bukuNullJudul = new Buku("9999999999", null, "Pengarang", 1, 100000.0);
        mockRepository.simpan(bukuNullJudul);

        List<Buku> hasil = mockRepository.cariByJudul("test");

        assertTrue(hasil.isEmpty());
    }

    @Test
    @DisplayName("Test cariByPengarang dengan pengarang null pada buku")
    void testCariByPengarangWithNullPengarangInBook() {
        Buku bukuNullPengarang = new Buku("9999999999", "Judul", null, 1, 100000.0);
        mockRepository.simpan(bukuNullPengarang);

        List<Buku> hasil = mockRepository.cariByPengarang("test");
